  ...
  _buildFilename(transcriptionId, timestamp) {
    if (timestamp) {
      ...
      if (!isNaN(d.getTime())) {
        ...
        return `PrivateFlow-${date}-${time}-${transcriptionId}.webm`;
      }
    }
    return `PrivateFlow-${transcriptionId}.webm`;
  }
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (stats.mtimeMs < cutoffMs) {
            fs.unlinkSync(filePath);
            // Extract ID from "PrivateFlow-...-{id}.webm" (or legacy "OpenWhispr-...-{id}.webm" / "{id}.webm")
